  ...
  static async generateReview(
    ...
    for (const { parentItem } of itemPdfPairs) {
      try {
        ...
        ztoolkit.log(
          `[Self-AI] 添加 AI-Reviewed 标签失败: ${parentItem.getField("title")}`,
          e,
        );
      }
    }
    ...
    const worker = async () => {
      while (queue.length > 0) {
        ...
        try {
          ...
          ztoolkit.log(
            `[Self-AI] 追加填表失败: ${task.parentItem.getField("title")}`,
            error,
          );
          const fallback =
            (await this.findTableNote(task.parentItem)) ||
    ...
    const worker = async () => {
      while (queue.length > 0) {
        ...
        try {
          ...
          ztoolkit.log(
            `[Self-AI] 填表失败: ${task.parentItem.getField("title")}`,
            error,
          );
          results.set(
            task.parentItem.id,
  ...
  static async attachPdfsToReport(
    ...
    for (const pdfAtt of pdfAttachments) {
      try {
        ...
        if (!filePath) {
          ztoolkit.log(`[Self-AI] PDF 附件无文件路径: ${pdfAtt.id}`);
          continue;
        }
        ...
        ztoolkit.log(`[Self-AI] 添加 PDF 附件失败:`, error);
        // 继续处理其他附件
      }
    }
  }
    ...
    for (let i = 0; i < pdfAttachments.length; i++) {
      ...
      try {
        ...
        try {
          ...
          ztoolkit.log(`[Self-AI] 读取 PDF 文件失败: ${filePath}`, e);
        }
        ...
        ztoolkit.log(
          `[Self-AI] 提取 PDF 内容失败: ${attachmentTitle}`,
          error,
        );
        // 继续处理其他文献
      }

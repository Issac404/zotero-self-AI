...
export class ImageSummaryService {
  ...
  public static async generateForItem(
    ...
    try {
      ...
      if (useExistingNote) {
        ...
        if (existingNote) {
          ...
          pdfContent = noteHtml
            ...
            .trim();
          ztoolkit.log(
            `[Self-AI] 使用已有 AI 笔记，长度: ${pdfContent.length}`,
          );
        } else {
          // 没有已有笔记，回退到 PDF 提取
          ztoolkit.log("[Self-AI] 未找到已有 AI 笔记，使用 PDF 提取");
      ...
      ztoolkit.log(
        `[Self-AI] 视觉摘要生成完成，长度: ${visualSummary.length}`,
      );
      ...
      ztoolkit.log(
        `[Self-AI] 图片生成完成，大小: ${Math.round(imageResult.imageBase64.length / 1024)} KB`,
      );
      ...
      if (error instanceof ImageGenerationError) {
        ...
        ztoolkit.log("[Self-AI] 一图总结生成失败:", error);
      }

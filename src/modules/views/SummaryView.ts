...
export class SummaryView extends BaseView {
  ...
  protected renderContent(): HTMLElement {
    ...
    queueButton.id = "self-ai-queue-button";
  ...
  private createChatContainer(): HTMLElement {
    ...
    chatButton.id = "self-ai-chat-toggle-button";
    ...
    this.chatSendButton.id = "self-ai-chat-send";
  ...
  private async handleChatSend(): Promise<void> {
    ...
    if (
      ...
      try {
        ...
        ztoolkit.log("[Self-AI] 包装聊天卡片失败:", e);
      }
    }
  ...
  private async saveChatToNote(
    ...
    try {
      ...
      ztoolkit.log("[Self-AI] 兼容保存对话到独立笔记失败:", error);
    }
  }
...
${jsonMarker}
<div id="self-ai-pair-${this.escapeHtml(pairId)}" style="margin-top:14px; padding-top:8px; border-top:1px dashed #ccc;">
...
`;
      ...
      ztoolkit.log("[Self-AI] 追问对已保存到独立笔记");
    } catch (e) {
      ztoolkit.log("[Self-AI] 保存追问对到独立笔记失败:", e);
    }
  }
  ...
  private async removeChatPairFromSeparateNote(pairId: string): Promise<void> {
    ...
    try {
      ...
      ztoolkit.log("[Self-AI] 从独立笔记删除追问对失败:", e);
    }
  }
  ...
  public async loadItemForChat(itemId: number): Promise<void> {
    try {
      ...
      try {
        ...
        if (pdfContent) {
          ...
          if (aiSummaryText) {
            try {
              ...
              ztoolkit.log("[Self-AI] 渲染 AI 总结卡片失败:", e);
            }
          } else {
            // 没有已有总结，显示欢迎提示
          ...
          try {
            ...
            ztoolkit.log("[Self-AI] 加载历史追问失败:", e);
          }
        ...
        this.hideLoading();
        ztoolkit.log("[Self-AI] 获取 PDF 内容失败:", err);
        new ztoolkit.ProgressWindow("AI Butler", {
          closeOnClick: true,
          closeTime: 3000,
        })
      ...
      this.hideLoading();
      ztoolkit.log("[Self-AI] loadItemForChat 失败:", err);
      this.clearPaperContext();
    }
  }
      ...
      try {
        ...
        if (pdfContent) {
          ...
          try {
            ...
            ztoolkit.log("[Self-AI] 渲染AI总结卡片失败:", e);
          }
        ...
        ztoolkit.log("[Self-AI] 获取PDF内容失败，无法启用追问功能:", err);
        this.clearPaperContext();
      }
    } catch (err) {
      this.hideLoading();
  ...
  private async loadExistingChatPairs(item: Zotero.Item): Promise<void> {
    try {
      ...
      for (const p of pairs) {
        ...
        if (this.outputContainer && userEl && asstEl) {
          ...
          try {
            ...
            ztoolkit.log("[Self-AI] 渲染历史聊天卡片失败:", e);
          }
        }
      ...
      ztoolkit.log("[Self-AI] 读取并恢复历史追问失败:", e);
    }
  }
  ...
  private initMathJax(): void {
    ...
    // 检查是否已添加 KaTeX CSS
    if (doc.getElementById("self-ai-katex-style")) {
    ...
    (async () => {
      try {
        ...
        if (!katexCss) {
          ztoolkit.log("[Self-AI] KaTeX CSS 加载失败，为空");
          return;
        }

        const styleEl = doc.createElement("style");
        styleEl.id = "self-ai-katex-style";
        ...
        if (insertTarget) {
          ...
          ztoolkit.log("[Self-AI] KaTeX CSS 已加载");
        }

        this.mathJaxReady = true;
      } catch (e) {
        ztoolkit.log("[Self-AI] KaTeX CSS 加载出错:", e);
      }
    })();
  }
  ...
  private injectSpinnerStyle(): void {
    ...
    // 检查是否已添加
    if (doc.getElementById("self-ai-spinner-style")) return;

    const style = doc.createElement("style");
    style.id = "self-ai-spinner-style";
    style.textContent = `
      @keyframes spin {
  ...
  public static convertMarkdownToHTMLCore(markdown: string): string {
    ...
    try {
      ...
      ztoolkit.log("[Self-AI][SummaryView] Markdown 解析错误:", error);
      ...
      (_match, _type, index) => {
        ...
        try {
          ...
          if (rendered.includes("katex-error")) {
            ztoolkit.log("[Self-AI] KaTeX 渲染有问题，内容:", content);
          }
          ...
          // KaTeX 渲染失败，显示原始公式
          ztoolkit.log("[Self-AI] KaTeX 渲染失败:", e, "公式内容:", content);
          if (isBlock) {

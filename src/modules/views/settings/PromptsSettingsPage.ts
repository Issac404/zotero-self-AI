...
export class PromptsSettingsPage {
  ...
  private getAllPresets(): PresetMap {
    ...
    setPref("customPrompts", JSON.stringify(builtins));
    return builtins;
  }
  ...
  private saveAsPreset(): void {
    ...
    const custom: PresetMap = this.getAllPresets();
  ...
  private deleteCustomPreset(): void {
    ...
    if (Object.keys(custom).length <= 1) {
      new ztoolkit.ProgressWindow("提示词")
        .createLine({ text: "至少需要保留一个预设", type: "default" })
        .show();
      return;
    }
    ...
    // 设置下拉框为第一个预设
    const nextKeys = Object.keys(custom);
    const nextKey = nextKeys[0];
    setTimeout(() => {
      (this.presetSelect as any).setValue(nextKey);
      this.loadPresetToEditor();
    }, 0);
  ...
  private resetDefault(): void {
    const ok = Services.prompt.confirm(
      Zotero.getMainWindow() as any,
      "恢复默认",
      "确定将所有提示词恢复为默认吗?此操作会清空所有您自定义的预设！",
    );
    if (!ok) return;

    setPref("customPrompts", "");
    ...
    new ztoolkit.ProgressWindow("提示词")
      .createLine({ text: "已恢复为全部默认模板", type: "success" })
      .show();
  }

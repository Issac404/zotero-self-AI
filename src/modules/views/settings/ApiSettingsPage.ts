...
export class ApiSettingsPage {
  ...
  public render(): void {
    ...
    const mineruUrlInput = this.createInput(
      "mineruServerUrl",
      "text",
    ...
    // 手动绑定保存事件
    let mineruSaveTimeout: ReturnType<typeof setTimeout> | null = null;
    const saveMineruUrl = () => {
      setPref("mineruServerUrl" as any, mineruUrlInput.value.trim());
    };

    mineruUrlInput.addEventListener("input", () => {
      if (mineruSaveTimeout) clearTimeout(mineruSaveTimeout);
      mineruSaveTimeout = setTimeout(saveMineruUrl, 500);
    });

    mineruUrlInput.addEventListener("blur", () => {
      if (mineruSaveTimeout) clearTimeout(mineruSaveTimeout);
      saveMineruUrl();
    });

    const mineruApiKeyInput = this.createInput(
      "mineruApiKey",
      "password",
      (getPref("mineruApiKey" as any) as string) || "",
      "输入 MinerU 的 API 密钥",
    );

    const saveMineruApiKey = () => {
      setPref("mineruApiKey" as any, mineruApiKeyInput.value.trim());
    };

    let mineruApiKeySaveTimeout: ReturnType<typeof setTimeout> | null = null;
    mineruApiKeyInput.addEventListener("input", () => {
      if (mineruApiKeySaveTimeout) clearTimeout(mineruApiKeySaveTimeout);
      mineruApiKeySaveTimeout = setTimeout(saveMineruApiKey, 500);
    });
    mineruApiKeyInput.addEventListener("blur", () => {
      if (mineruApiKeySaveTimeout) clearTimeout(mineruApiKeySaveTimeout);
      saveMineruApiKey();
    });
    ...
    sectionMineru.appendChild(
      this.createFormGroup(
        "MinerU API Key",
        mineruApiKeyInput,
        "MinerU 服务的 API 密钥（如服务端不需要认证可留空）",
      ),
    );
    sectionMineru.appendChild(
      this.createFormGroup(
        "MinerU 服务器地址 *",
        mineruUrlInput,
        "【必填】本地部署的 MinerU OCR 服务地址，如 http://192.168.137.11:18000",
      ),
    );

...
export abstract class BaseView {
  ...
  protected applyTheme(): void {
    ...
    try {
      // 确保容器具备主题作用域（若根上已添加 self-ai-root，也可不必）
      if (!this.container.classList.contains("self-ai-root")) {
        this.container.classList.add("self-ai-root");
      }
    } catch {
      // Ignored
    }
    ...
    if (isDark) {
      this.container.classList.add("self-ai-dark");
      ztoolkit.log(`[AI Butler] 已应用暗色主题到 ${this.viewId}`);
    } else {
      this.container.classList.remove("self-ai-dark");

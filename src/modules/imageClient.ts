...
export class ImageClient {
  ...
  private static async generateImageViaOpenAI(
    ...
    ztoolkit.log(`[Self-AI] 调用 OpenAI 兼容生图 API: ${endpoint}`);
    ztoolkit.log(`[Self-AI] 生图提示词长度: ${prompt.length} 字符`);
    ...
    try {
      ...
      if (extracted) {
        ztoolkit.log(
          `[Self-AI] 成功生成图片，MIME: ${extracted.mimeType}, 大小: ${Math.round(extracted.imageBase64.length / 1024)} KB`,
        );
        return extracted;
      }
      ...
      if (remoteUrl) {
        ...
        ztoolkit.log(
          `[Self-AI] 成功下载图片，MIME: ${downloaded.mimeType}, 大小: ${Math.round(downloaded.imageBase64.length / 1024)} KB`,
        );
        return downloaded;
      }
  ...
  public static async generateImage(
    ...
    ztoolkit.log(
      `[Self-AI] 调用 Gemini 生图 API: ${endpoint}, 提示词: ${prompt.length} 字符`,
    );
    ...
    ztoolkit.log(
      `[Self-AI] 生图响应完成, 耗时 ${elapsedSec}s, 大小: ${Math.round((response.response?.length || 0) / 1024)} KB`,
    );
    ...
    try {
      ...
      ztoolkit.log(
        `[Self-AI] 成功生成图片，MIME: ${mimeType}, 大小: ${Math.round(imageBase64.length / 1024)} KB`,
      );

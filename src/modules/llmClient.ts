...
export class LLMClient {
  ...
  private static buildOptions(
    ...
    // Sanitize to prevent "An invalid or illegal string was specified" in HTTP headers
    if (common.apiKey) {
      common.apiKey = common.apiKey.replace(/[^\x20-\x7E]/g, "");
    }
    if (common.apiUrl) {
      // Allow standard URL characters
      common.apiUrl = common.apiUrl.replace(/[^\x20-\x7E]/g, "");
    }
   ...
   * 多模态（Vision）摘要生成
   *
   * 将预组装的多模态内容数组（文本 + 图片）直接发送给 LLM。
   * 用于 MinerU OCR 解析后的 Markdown + 切片图片场景。
   *
   * @param contentParts OpenAI Vision 兼容的 content 数组
   * @param onProgress 流式输出回调
   * @returns AI 生成的摘要内容
   */
  static async generateVisionSummary(
    contentParts: Array<{
      type: string;
      text?: string;
    ...
    // 直接传递多模态 content 数组作为 "content"
    // Provider 的 generateSummary 会检测 content 是否为数组
    return impl.generateSummary(
      contentParts,
      false, // isBase64 = false，因为我们直接传数组
      undefined, // prompt 已包含在 contentParts 中
      options,
      onProgress as ProviderProgressCb,
    );
  }

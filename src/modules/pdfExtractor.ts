...
export class PDFExtractor {
  ...
  public static async getPdfFileSize(item: Zotero.Item): Promise<number> {
    try {
      if (item.isAttachment()) {
        if (!item.isPDFAttachment()) return 0;
  ...
  public static async extractTextFromItem(item: Zotero.Item): Promise<string> {
    ...
    if (!item.isAttachment()) {
      ...
      // 按 dateAdded 升序排序 (最早的在前)
      pdfAttachment = pdfAttachments.sort((a, b) => {
        const dateA = new Date(a.dateAdded).getTime();
        const dateB = new Date(b.dateAdded).getTime();
        return dateA - dateB;
      })[0];
      ...
      if (!item.isPDFAttachment()) {
        throw new Error("The provided item is not a PDF attachment");
      }
      pdfAttachment = item;
      ztoolkit.log(
        `[AI Butler] Directly using provided PDF attachment: ${pdfAttachment.getField("title")}`,
      );
    }

    // 第三步:从 PDF 附件中提取文本
    const currentPdfMode = (getPref("pdfProcessMode") as string) || "base64";
    const mineruServerUrl = (getPref("mineruServerUrl" as any) as string) || "";
    // 若选择 MinerU 模式且已配置服务器地址，则使用本地 MinerU 服务提取文本
    if (
      currentPdfMode === "mineru" &&
      mineruServerUrl &&
      mineruServerUrl.trim().length > 0
    ) {
      ztoolkit.log(
        "[AI Butler] MinerU pdf process mode selected, routing to local MinerU service...",
      );
      try {
        const { MineruClient } = await import("./mineruIntegration");
        const result = await MineruClient.parseLocalPdf(item);
  ...
  public static async extractBase64FromItem(
    ...
    if (!item.isAttachment()) {
      ...
      pdfAttachment = item;
      ztoolkit.log(
        `[AI Butler] Directly using provided PDF attachment (Base64): ${pdfAttachment.getField("title")}`,
      );
    }
    ...
    try {
      ...
      if (typeof IOUtils !== "undefined") {
        bytes = await IOUtils.read(pdfPath);
      } else {
        const { OS } = Zotero.getMainWindow() as any;
        bytes = await OS.File.read(pdfPath);
      }

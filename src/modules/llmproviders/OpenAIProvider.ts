...
export class OpenAIProvider implements ILlmProvider {
  ...
  async generateSummary(
            ...
            requestObserver: (xmlhttp: XMLHttpRequest) => {
              xmlhttp.onprogress = (e: any) => {
                ...
                try {
                        ...
                        if (
                          ...
                          if (onProgress && current.length > delivered) {
                            ...
                            Promise.resolve(onProgress(newChunk)).catch((err) =>
                              ztoolkit.log(
                                "[Self-AI] onProgress error (OpenAI Responses SSE):",
                                err,
                              ),
                            );
                          }
                  ...
                  ztoolkit.log(
                    "[Self-AI] OpenAI Responses SSE parse error:",
                    err,
                  );
                }
              };
    ...
    const input: any[] = [
      ...
      {
        role: "user",
        content: buildUserMessage(prompt || "", content as string),
      },
    ];
          ...
          requestObserver: (xmlhttp: XMLHttpRequest) => {
            xmlhttp.onprogress = (e: any) => {
              ...
              try {
                    ...
                    try {
                      ...
                      if (typeof delta === "string" && delta.length > 0) {
                        ...
                        if (onProgress && current.length > delivered) {
                          ...
                          Promise.resolve(onProgress(newChunk)).catch((err) =>
                            ztoolkit.log(
                              "[Self-AI] onProgress callback error:",
                              err,
                            ),
                          );
                        }
                ...
                ztoolkit.log("[Self-AI] stream parse error:", err);
              }
            };
            xmlhttp.onerror = () => {
              abortedDueToError = true;
  ...
  async chat(
          ...
          requestObserver: (xmlhttp: XMLHttpRequest) => {
            xmlhttp.onprogress = (e: any) => {
              ...
              try {
                    ...
                    try {
                      ...
                      if (
                        ...
                        if (onProgress && current.length > delivered) {
                          ...
                          Promise.resolve(onProgress(newChunk)).catch((err) =>
                            ztoolkit.log(
                              "[Self-AI] onProgress error (OpenAI Responses chat SSE):",
                              err,
                            ),
                          );
                        }
                ...
                ztoolkit.log(
                  "[Self-AI] OpenAI Responses chat SSE parse error:",
                  err,
                );
              }
            };
    ...
    try {
          ...
          xmlhttp.onprogress = (e: any) => {
            ...
            if (status >= 400) {
              try {
                ...
                ztoolkit.log("[Self-AI] OpenAI HTTP error:", {
                  status,
                  code,
                  msg,
            ...
            try {
                  ...
                  try {
                    ...
                    if (delta) {
                      ...
                      if (onProgress && current.length > delivered) {
                        ...
                        delivered = current.length;
                        Promise.resolve(onProgress(newChunk)).catch((err) => {
                          ztoolkit.log("[Self-AI] onProgress error:", err);
                        });
                      }
                    }
                  } catch {
              ...
              ztoolkit.log("[Self-AI] OpenAI stream parse error:", err);
            }
          };
          xmlhttp.onerror = () => {
            if (!abortError)
      ...
      ztoolkit.log("[Self-AI] OpenAI request error:", {
  ...
  async generateMultiFileSummary(
    ...
    for (let i = 0; i < pdfFiles.length; i++) {
      ...
      if (pdfFile.base64Content && pdfFile.base64Content.length > 0) {
        ...
        ztoolkit.log(
          `[Self-AI] 添加 PDF 附件 (${i + 1}/${pdfFiles.length}): ${pdfFile.displayName}, base64 长度: ${pdfFile.base64Content.length}`,
        );
      } else {
        ztoolkit.log(
          `[Self-AI] PDF 文件 ${pdfFile.displayName} 无 base64 内容，跳过`,
        );
      }
    }
    ...
    ztoolkit.log(
      `[Self-AI] 准备发送 ${fileParts.length} 个 PDF 附件到 OpenAI`,
    );
        ...
        requestObserver: (xmlhttp: XMLHttpRequest) => {
          xmlhttp.onprogress = (e: any) => {
            ...
            try {
                  ...
                  try {
                    ...
                    if (
                      ...
                      if (onProgress && current.length > delivered) {
                        ...
                        Promise.resolve(onProgress(newChunk)).catch((err) =>
                          ztoolkit.log(
                            "[Self-AI] onProgress error (OpenAI multi-PDF):",
                            err,
                          ),
                        );
                      }
              ...
              ztoolkit.log(
                "[Self-AI] OpenAI multi-PDF SSE parse error:",
                err,
              );
            }
          };

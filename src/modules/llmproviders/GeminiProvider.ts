...
export class GeminiProvider implements ILlmProvider {
  ...
  async generateSummary(
    ...
    if (isBase64) {
      ...
      const userContent = buildUserMessage(prompt || "", content as string);
      const generationConfig: any = {};
      if (options.temperature !== undefined)
        generationConfig.temperature = temperature;
      if (options.topP !== undefined) generationConfig.topP = topP;
        ...
        requestObserver: (xmlhttp: XMLHttpRequest) => {
          xmlhttp.onprogress = (e: any) => {
            ...
            try {
              ...
              ztoolkit.log("[Self-AI] Gemini stream parse error:", err);
            }
          };
        },
      });
  ...
  async chat(
    ...
    try {
          ...
          xmlhttp.onprogress = (e: any) => {
            ...
            if (status >= 400) {
              try {
                ...
                ztoolkit.log("[Self-AI] Gemini HTTP error:", {
                  status,
                  code,
                  msg,
      ...
      ztoolkit.log("[Self-AI] Gemini request error:", {
  ...
  private async uploadFileToGemini(
    ...
    try {
      ...
      ztoolkit.log("[Self-AI] Gemini 文件上传初始化失败:", error);
    ...
    try {
      ...
      ztoolkit.log(`[Self-AI] 文件上传成功: ${displayName} -> ${fileUri}`);
      return fileUri;
    } catch (error: any) {
      ztoolkit.log("[Self-AI] Gemini 文件上传失败:", error);
  ...
  async generateMultiFileSummary(
    ...
    ztoolkit.log(
      `[Self-AI] 准备发送 ${fileParts.length} 个 PDF 附件到 Gemini`,
    );

...
export class OpenAICompatProvider implements ILlmProvider {
  ...
  async generateSummary(
    ...
    if (Array.isArray(content)) {
      // 多模态内容数组（由 generateVisionSummary 传入）
      // content 已经是 OpenAI Vision 兼容的 content 数组，直接使用
      messages.push({
        role: "user",
        content: content,
      });
          ...
          requestObserver: (xmlhttp: XMLHttpRequest) => {
            xmlhttp.onprogress = (e: any) => {
              ...
              try {
                      ...
                      if (typeof delta === "string" && delta.length > 0) {
                        ...
                        if (onProgress && current.length > delivered) {
                          ...
                          Promise.resolve(onProgress(newChunk)).catch((err) =>
                            ztoolkit.log(
                              "[Self-AI] onProgress error (OpenAI Compat SSE):",
                              err,
                            ),
                          );
                        }
                ...
                ztoolkit.log("[Self-AI] OpenAI Compat SSE parse error:", err);
              }
            };
            xmlhttp.onerror = () => {
              if (!abortError)
        ...
        requestObserver: (xmlhttp: XMLHttpRequest) => {
          xmlhttp.onprogress = (e: any) => {
            ...
            try {
                  ...
                  try {
                    ...
                    if (typeof delta === "string" && delta.length > 0) {
                      ...
                      if (onProgress && current.length > delivered) {
                        ...
                        Promise.resolve(onProgress(newChunk)).catch((err) =>
                          ztoolkit.log(
                            "[Self-AI] onProgress error (OpenAI Compat chat SSE):",
                            err,
                          ),
                        );
                      }
              ...
              ztoolkit.log(
                "[Self-AI] OpenAI Compat chat SSE parse error:",
                err,
              );
            }
          };
  ...
  async generateMultiFileSummary(
    ...
    ztoolkit.log(
      `[Self-AI] 准备发送 ${fileParts.length} 个 PDF 附件到 OpenAI 兼容接口`,
    );
        ...
        requestObserver: (xmlhttp: XMLHttpRequest) => {
          xmlhttp.onprogress = (e: any) => {
            ...
            try {
                  ...
                  try {
                    ...
                    if (typeof delta === "string" && delta.length > 0) {
                      ...
                      if (onProgress && current.length > delivered) {
                        ...
                        Promise.resolve(onProgress(newChunk)).catch((err) =>
                          ztoolkit.log(
                            "[Self-AI] onProgress error (OpenAI Compat multi-PDF):",
                            err,
                          ),
                        );
                      }
              ...
              ztoolkit.log(
                "[Self-AI] OpenAI Compat multi-PDF SSE parse error:",
                err,
              );
            }
          };

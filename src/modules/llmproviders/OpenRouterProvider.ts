...
export class OpenRouterProvider implements ILlmProvider {
  ...
  async generateSummary(
    ...
    if (isBase64) {
      ...
      const userText = buildUserMessage(prompt || "", content as string);
        ...
        requestObserver: (xmlhttp: XMLHttpRequest) => {
          xmlhttp.onprogress = (e: any) => {
            ...
            try {
                    ...
                    if (typeof delta === "string" && delta.length > 0) {
                      ...
                      if (onProgress && current.length > delivered) {
                        ...
                        Promise.resolve(onProgress(newChunk)).catch((err) =>
                          ztoolkit.log(
                            "[Self-AI] onProgress error (OpenRouter SSE):",
                            err,
                          ),
                        );
                      }
              ...
              ztoolkit.log("[Self-AI] OpenRouter SSE parse error:", err);
            }
          };
          xmlhttp.onerror = () => {
            if (!abortError)
  ...
  async generateMultiFileSummary(
    ...
    ztoolkit.log(
      `[Self-AI] 准备发送 ${fileParts.length} 个 PDF 附件到 OpenRouter`,
    );

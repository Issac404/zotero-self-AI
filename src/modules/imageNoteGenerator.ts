...
export class ImageNoteGenerator {
  ...
  public static async createImageNote(
    ...
    if (!note) {
      ...
      await note.saveTx();
      ztoolkit.log(`[Self-AI] 创建新的一图总结笔记: ${note.id}`);
    }
    ...
    ztoolkit.log(
      `[Self-AI] 创建图片附件: key=${attachment.key}, noteID=${note.id}`,
    );
    ...
    ztoolkit.log(
      `[Self-AI] ${isUpdate ? "更新" : "创建"}一图总结笔记完成: ${note.id}`,
    );
    return note;
  }
...
<p style="text-align: center; color: #666; font-size: 12px;">
  ...
  public static async createImageNoteFromFile(
    ...
    try {
      ...
      ztoolkit.log(`[Self-AI] 从文件创建一图总结笔记失败:`, error);
  ...
  public static async findExistingImageNote(
    ...
    try {
      ...
      ztoolkit.log(`[Self-AI] 查找一图总结笔记，共 ${noteIDs.length} 个笔记`);
      ...
      for (const nid of noteIDs) {
        ...
        if (hasTag || titleMatch1 || titleMatch2 || titleMatch3) {
          ztoolkit.log(
            `[Self-AI] 找到一图总结笔记: ID=${nid}, hasTag=${hasTag}, titleMatch1=${titleMatch1}, titleMatch2=${titleMatch2}, titleMatch3=${titleMatch3}`,
          );
          return n as Zotero.Item;
        }
      }

      ztoolkit.log(`[Self-AI] 未找到一图总结笔记`);
      return null;
    } catch (error) {
      ztoolkit.log(`[Self-AI] 查找一图总结笔记失败:`, error);
      return null;
    }
  }
  ...
  public static extractImageFromNote(note: Zotero.Item): string | null {
    try {
      ...
      ztoolkit.log(
        "[Self-AI] 笔记中未找到图片，HTML 内容:",
        noteHtml.substring(0, 500),
      );
      return null;
    } catch (error) {
      ztoolkit.log("[Self-AI] 提取笔记图片失败:", error);
      return null;
    }
  }
  ...
  public static extractAttachmentKeyFromNote(note: Zotero.Item): string | null {
    try {
      ...
      if (match && match[1]) {
        ztoolkit.log(`[Self-AI] 找到附件 key: ${match[1]}`);
        return match[1];
      }

      return null;
    } catch (error) {
      ztoolkit.log("[Self-AI] 提取附件 key 失败:", error);
      return null;
    }
  }
    ...
    try {
      ...
      if (attachmentKey) {
        ...
        if (attachment && (attachment as any).isFileAttachment?.()) {
          ...
          if (filePath) {
            ztoolkit.log(`[Self-AI] 附件文件路径: ${filePath}`);
      ...
      ztoolkit.log("[Self-AI] 无法从笔记获取图片");
      return null;
    } catch (error) {
      ztoolkit.log("[Self-AI] 获取笔记图片失败:", error);
      return null;
    }
  }
    ...
    try {
      ...
      if (attachmentKey) {
        ...
        if (attachment && (attachment as any).isFileAttachment?.()) {
          const filePath = await (attachment as any).getFilePathAsync?.();
          if (filePath) {
            ztoolkit.log(`[Self-AI] 获取图片附件路径: ${filePath}`);
            return filePath;
          }
        }
      }

      ztoolkit.log("[Self-AI] 无法获取图片附件路径");
      return null;
    } catch (error) {
      ztoolkit.log("[Self-AI] 获取图片附件路径失败:", error);
      return null;
    }
  }

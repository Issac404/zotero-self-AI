...
export async function registerPrefsScripts(_window: Window) {
  ...
  const logError = (label: string, error: any) => {
    try {
      ...
      slog(`[Self-AI][Prefs] ${label} failed: ${name} ${message}`);
      if (stack) {
        slog(`[Self-AI][Prefs] ${label} stack:`, stack);
      }
    } catch {
      // ignore
    }
...
function updatePrefsUI(win: Window) {
  ...
  const apiKeyInput = doc.getElementById(
    "zotero-prefpane-self-ai-openaiApiKey",
  ) as HTMLInputElement | null;
  const apiUrlInput = doc.getElementById(
    "zotero-prefpane-self-ai-openaiApiUrl",
  ) as HTMLInputElement | null;
  const modelInput = doc.getElementById(
    "zotero-prefpane-self-ai-openaiApiModel",
  ) as HTMLInputElement | null;
  const temperatureInput = doc.getElementById(
    "zotero-prefpane-self-ai-temperature",
  ) as HTMLInputElement | null;
  const promptTextarea = doc.getElementById(
    "zotero-prefpane-self-ai-summaryPrompt",
  ) as HTMLTextAreaElement | null;
  const streamCheckbox = doc.getElementById(
    "zotero-prefpane-self-ai-stream",
  ) as HTMLInputElement | null;
    ...
    try {
      ...
      if (key === "summaryPrompt") {
        ...
        if (shouldUpdatePrompt(currentPromptVersion, currentPrompt)) {
          ztoolkit.log(`[Self-AI][Prefs] 更新提示词到版本 ${PROMPT_VERSION}`);
      ...
      if (currentValue === undefined || currentValue === null) {
        ...
        //   : defaultValue;
        // ztoolkit.log(`[Self-AI][Prefs] 初始化配置: ${key} = ${preview}`);
        ...
        // 对于字符串类型，如果是空字符串也重置
        // ztoolkit.log(`[Self-AI][Prefs] 重置空配置: ${key}`);
        setPref(key as any, defaultValue);
      }
    } catch (error) {
      ztoolkit.log(`[Self-AI][Prefs] 初始化配置失败: ${key}`, error);
      // 如果读取失败，尝试强制设置
      try {
        setPref(key as any, defaultValue);
      } catch (e) {
        ztoolkit.log(`[Self-AI][Prefs] 强制设置配置失败: ${key}`, e);
      }
    }
  }
}
...
function diagnosePrefs() {
  ztoolkit.log("[Self-AI][Prefs] ========== 配置诊断开始 ==========");
  ...
  for (const key of keys) {
    try {
      ...
      ztoolkit.log(`[Self-AI][Prefs] 配置项: ${key}`);
      ...
      ztoolkit.log(`[Self-AI][Prefs] 读取配置失败: ${key}`, error);
    }
  }

  ztoolkit.log("[Self-AI][Prefs] ========== 配置诊断结束 ==========");
}
...
function bindOpenMainWindowButton(win: Window) {
  try {
    ...
    const tryBind = () => {
      ...
      if (!button) {
        if (attempts === 1) {
          slog(`[Self-AI][Prefs] Waiting for button: ${buttonId}`);
        }
        ...
        slog(`[Self-AI][Prefs] Button not found after retries: ${buttonId}`);
        return;
      }

      // Remove any previously bound handler (e.g. when the pane reloads)
      const oldListener = (button as any).__selfAIListener;
      if (oldListener) {
        try {
    ...
    try {
      ztoolkit?.log?.("[Self-AI][Prefs] 绑定按钮事件时出错:", error);
    } catch {
      console.error("[Self-AI][Prefs] 绑定按钮事件时出错:", error);
    }
  }
}

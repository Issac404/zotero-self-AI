...
function scheduleSidebarRefresh(target: SidebarAutoRefreshTarget): void {
  ...
  sidebarRefreshTimer = setTimeout(() => {
    void runSidebarRefresh().catch((e) => {
      ztoolkit.log("[Self-AI] Sidebar auto-refresh failed:", e);
    });
...
async function runSidebarRefresh(): Promise<void> {
  ...
  if (targets.includes("imageSummary")) {
    const imageContainer = doc.getElementById(
      "self-ai-image-container",
    ) as HTMLElement | null;
    const imageBtnContainer = doc.getElementById(
      "self-ai-image-btn-container",
    ) as HTMLElement | null;
...
async function ensureSidebarAutoRefresh(): Promise<void> {
  ...
  try {
    ...
    ztoolkit.log("[Self-AI] Sidebar auto-refresh bind failed:", error);
  }
}
...
function renderActionButtons(
  ...
  fullChatBtn.addEventListener("click", async () => {
    try {
      ...
      ztoolkit.log("[Self-AI] 完整追问按钮点击失败:", error);
    }
  });
  ...
  quickChatBtn.id = "self-ai-quick-chat-btn";

  // 刷新按钮
  const refreshBtn = doc.createElement("button");
  refreshBtn.id = "self-ai-refresh-btn";
...
function renderNoteSection(
  ...
  noteSection.className = "self-ai-note-section";
  ...
  noteHeader.className = "self-ai-note-header";
  ...
  noteContentWrapper.className = "self-ai-note-content-wrapper";
  ...
  noteContent.className = "self-ai-note-content markdown-body";
  noteContent.id = "self-ai-note-content";
  ...
  copyBtn.id = "self-ai-copy-note-btn";
  ...
  copyBtn.addEventListener("click", async (e: Event) => {
    ...
    try {
      ...
      ztoolkit.log("[Self-AI] 复制笔记失败:", err);
      copyBtn.textContent = "❌";
      setTimeout(() => {
        copyBtn.textContent = "📋";
      }, 1500);
...
async function loadTableContent(
  ...
  try {
    ...
    ztoolkit.log("[Self-AI] 加载表格内容失败:", error);
...
function renderTableSection(
  ...
  tableSection.className = "self-ai-table-section";
  ...
  refillBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      ztoolkit.log("[Self-AI] 重新填表失败:", err);
    } finally {
      refillBtn.textContent = "🔄 重新生成";
  ...
  tableContentEl.id = "self-ai-table-content";
...
function renderImageSummarySection(
  ...
  imageSummarySection.className = "self-ai-image-summary-section";
  ...
  imageBtnContainer.id = "self-ai-image-btn-container";
  ...
  imageContainer.id = "self-ai-image-container";
...
function renderMindmapSection(
  ...
  mindmapSection.className = "self-ai-mindmap-section";
  ...
  mindmapContainer.id = "self-ai-mindmap-container";
...
async function loadMindmapContent(
  ...
  try {
    ...
    if (mindmapWin?.__selfAIMindmapMessageHandler) {
      try {
        mindmapWin.removeEventListener(
          "message",
          mindmapWin.__selfAIMindmapMessageHandler,
        );
      } catch {
        // ignore
      }
      mindmapWin.__selfAIMindmapMessageHandler = null;
    }
    ...
    ztoolkit.log("[Self-AI] 思维导图笔记 HTML 长度:", noteHtml.length);
    ztoolkit.log(
      "[Self-AI] 思维导图正则匹配结果:",
      match ? "匹配成功" : "匹配失败",
    );
    if (match) {
      ztoolkit.log("[Self-AI] 匹配的内容长度:", match[1]?.length);
    } else {
      // 尝试调试：检查是否包含 markmap 关键字
      ztoolkit.log(
        "[Self-AI] 笔记是否包含 markmap:",
        noteHtml.includes("markmap"),
      );
      ztoolkit.log("[Self-AI] 笔记是否包含 ```:", noteHtml.includes("```"));
      // 尝试查找 markmap 位置
      const markmapIdx = noteHtml.indexOf("markmap");
      if (markmapIdx >= 0) {
        ztoolkit.log(
          "[Self-AI] markmap 周围内容:",
    ...
    try {
      ...
      iframeWrapper.id = "self-ai-mindmap-wrapper";
      ...
      iframe.id = "self-ai-mindmap-iframe";
      ...
      const messageHandler = async (event: MessageEvent) => {
        if (event.data && event.data.type === "mindmap-ready") {
          ztoolkit.log(
            "[Self-AI] 收到 iframe ready 消息，发送 markdown 数据",
          );
          try {
            ...
            ztoolkit.log("[Self-AI] 已发送 markdown 数据到 iframe");
          } catch (e) {
            ztoolkit.log("[Self-AI] 发送数据到 iframe 失败:", e);
          }
        }

        if (event.data && event.data.type === "open-mindmap-viewer") {
          ztoolkit.log("[Self-AI] 收到打开思维导图预览窗口请求");
          try {
            await openMindmapViewerWindow(mdContent, targetItem);
          } catch (e) {
            ztoolkit.log("[Self-AI] 打开思维导图预览窗口失败:", e);
        ...
        if (event.data && event.data.type === "export-mindmap") {
          ztoolkit.log("[Self-AI] 收到导出请求, 格式:", event.data.format);
          try {
            ...
            if (customPath && customPath.trim()) {
              ...
              try {
                ...
                ztoolkit.log("[Self-AI] 自定义目录创建失败，回退到桌面:", e);
                downloadDir = "";
              }
            }
            ...
            if (!downloadDir) {
              try {
                ...
                ztoolkit.log(
                  "[Self-AI] 无法获取桌面目录，使用 Zotero 数据目录:",
                  e,
                );
                // 回退到 Zotero 数据目录
            ...
            ztoolkit.log("[Self-AI] 思维导图已保存到:", filePath);
            ...
            ztoolkit.log("[Self-AI] 保存思维导图失败:", e);
      ...
      if (mindmapWin) {
        mindmapWin.__selfAIMindmapMessageHandler = messageHandler;
        mindmapWin.addEventListener("message", messageHandler);
      }

      // 监听 iframe 加载完成（备用方案）
      iframe.addEventListener("load", () => {
        ztoolkit.log("[Self-AI] mindmap.html 加载完成");

        // 备用：如果 500ms 内没收到 ready 消息，直接发送
        setTimeout(() => {
          try {
            ztoolkit.log("[Self-AI] 备用方案：直接发送数据");
            iframe.contentWindow?.postMessage(
              {
                type: "render-mindmap",
                markdown: mdContent,
      ...
      ztoolkit.log("[Self-AI] 思维导图 iframe 创建成功");
    } catch (renderError: any) {
      ztoolkit.log("[Self-AI] 思维导图渲染失败:", renderError);
    ...
    ztoolkit.log("[Self-AI] 加载思维导图失败:", err);
...
function renderChatArea(
  ...
  chatArea.id = "self-ai-inline-chat";
      ...
      if (chatArea.style.display === "none") {
        ...
        if (!currentChatState.pdfContent && item) {
          try {
            ...
            ztoolkit.log("[Self-AI] 快速提问加载 PDF 失败:", err);
    ...
    try {
      ...
      saveBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          ztoolkit.log("[Self-AI] 保存快速提问对话失败:", err);
      ...
      ztoolkit.log("[Self-AI] 快速提问发送失败:", err);
...
async function getOrCreateChatNote(item: Zotero.Item): Promise<Zotero.Item> {
  ...
  for (const nid of noteIDs) {
    try {
      ...
      if (!n) continue;
      const tags: Array<{ tag: string }> = (n as any).getTags?.() || [];
      const hasChatTag = tags.some((t) => t.tag === "Self-AI-Chat");
...
async function saveChatPairToNote(
  ...
  if (noteHtml.includes(`AI_BUTLER_CHAT_PAIR_START id=${pairId}`)) {
    ztoolkit.log("[Self-AI] 该对话对已保存过，跳过重复保存");
    return;
  }
...
${jsonMarker}
<div id="self-ai-pair-${escapeHtmlForNote(pairId)}" style="margin-top:14px; padding-top:8px; border-top:1px dashed #ccc;">
...
`;
  ...
  ztoolkit.log("[Self-AI] 快速提问对话已保存到笔记");
}
  ...
  try {
    ...
    for (const nid of noteIDs) {
      try {
        ...
        const isChatNote =
          tags.some((t) => t.tag === "Self-AI-Chat") ||
    ...
    if (parserError) {
      ...
      ztoolkit.log(
        `[Self-AI] XML Parsing Error: ${errorText}`,
        errorLocation,
      );
      ...
      copyBtn.addEventListener("click", () => {
        try {
          ...
          ztoolkit.log("[Self-AI] Copy failed:", e);
    ...
    ztoolkit.log("[Self-AI] 加载笔记失败:", err);
  ...
  try {
    ...
    downloadBtn.addEventListener("click", async () => {
      try {
        ...
        ztoolkit.log("[Self-AI] 下载图片失败:", err);
        new ztoolkit.ProgressWindow("AI Butler", {
          closeOnClick: true,
          closeTime: 3000,
        })
    ...
    openFolderBtn.addEventListener("click", async () => {
      try {
        ...
        ztoolkit.log("[Self-AI] 打开文件夹失败:", err);
        new ztoolkit.ProgressWindow("AI Butler", {
          closeOnClick: true,
          closeTime: 3000,
        })
    ...
    ztoolkit.log("[Self-AI] 加载一图总结失败:", err);
...
async function openImageSummaryViewerWindow(
  ...
  try {
    (dialogWin as any).__selfAIImageDataUri = imageDataUri;
    (dialogWin as any).__selfAITitle = title;
  } catch {
    // ignore
  }
...
async function openMindmapViewerWindow(
  ...
  // Extra fallback channel in case window.arguments isn't available for some reason
  try {
    (dialogWin as any).__selfAIMindmapMarkdown = markdown;
    (dialogWin as any).__selfAITitle = title;
    (dialogWin as any).__selfAIPrefsPrefix = config.prefsPrefix;
  } catch {
    // ignore
  }
...
async function getNoteMarkdownContent(
  ...
  try {
    ...
    ztoolkit.log("[Self-AI] 获取笔记 Markdown 内容失败:", err);
    return null;
  }
}
...
async function copyToClipboard(doc: Document, text: string): Promise<void> {
  try {
    ...
    ztoolkit.log("[Self-AI] 复制到剪贴板失败:", err);
    throw err;
  }
}

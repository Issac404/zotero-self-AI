...
function registerPrefsPane() {
  const prefOptions = {
    ...
    image: `chrome://${config.addonRef}/content/icons/logo.png`, // 面板图标
...
function initializeDefaultPrefsOnStartup() {
  ...
  const defaults: Record<string, any> = {
    ...
    // MinerU 本地 OCR 服务
    mineruServerUrl: "http://192.168.137.11:18000", // 本地 MinerU 服务器地址
  };
  ...
  for (const [key, defaultValue] of Object.entries(defaults)) {
    try {
      ...
      // 配置读取失败时记录错误
      ztoolkit.log(`[Self-AI] 启动时初始化配置失败: ${key}`, error);

      // 尝试强制设置默认值
      try {
        setPref(key as any, defaultValue);
      } catch (e) {
        ztoolkit.log(`[Self-AI] 启动时强制设置配置失败: ${key}`, e);
      }
    }
  }
}
 ...
 */
async function openSelfAIDashboardFromUnifiedEntry(): Promise<void> {
...
function registerContextMenuItem() {
  // 获取插件图标路径,用于菜单项显示
  const menuIcon = `chrome://${config.addonRef}/content/icons/logo.png`;
  ...
  menu.register("item", {
    ...
    commandListener: async (_ev: Event) => {
      await openSelfAIDashboardFromUnifiedEntry();
    },
...
function registerLibraryToolbarButton(win: Window) {
  try {
    ...
    const buttonId = "self-ai-library-toolbar-btn";
    ...
    if (!toolbar) {
      ztoolkit.log("[Self-AI] 找不到文献库工具栏");
      return;
    }
    ...
    button.setAttribute(
      "tooltiptext",
      getString("library-toolbar-self-ai" as any),
    );
    ...
    button.addEventListener("click", async () => {
      try {
        ...
        ztoolkit.log("[Self-AI] 打开 AI 管家失败:", error);
        new ztoolkit.ProgressWindow("Self AI", {
          closeOnClick: true,
          closeTime: 3000,
        })
          .createLine({
    ...
    ztoolkit.log("[Self-AI] 文献库工具栏按钮已添加");
  } catch (error) {
    ztoolkit.log("[Self-AI] 注册文献库工具栏按钮失败:", error);
  }
}
...
function registerReaderToolbarButton() {
  ...
  const createToolbarButton = (doc: Document, reader: any) => {
    ...
    buttonContainer.className = "self-ai-toolbar-container";
    ...
    button.className = "toolbar-button self-ai-reader-chat-btn";
    button.innerHTML = `🤖`;
    ...
    button.addEventListener("click", async () => {
      try {
        ...
        ztoolkit.log("[Self-AI] Reader 工具栏按钮点击失败:", error);
        new ztoolkit.ProgressWindow("Self AI", {
          closeOnClick: true,
          closeTime: 3000,
        })
    ...
    setTimeout(() => {
      try {
        ...
        for (const reader of readers) {
          ...
          // 检查是否已经添加过按钮
          if (toolbar.querySelector(".self-ai-toolbar-container")) continue;
        ...
        ztoolkit.log(
          `[Self-AI] 已为 ${readers.length} 个已打开的 Reader 添加工具栏按钮`,
        );
      } catch (err) {
        ztoolkit.log("[Self-AI] 处理已打开的 Reader 失败:", err);
      }
    }, 1000);

    ztoolkit.log("[Self-AI] Reader 工具栏按钮已注册");
  } catch (error) {
    ztoolkit.log("[Self-AI] 注册 Reader 工具栏按钮失败:", error);
  }
}
...
async function handleOpenAIChat(itemId: number): Promise<void> {
  try {
    ...
    ztoolkit.log("[Self-AI] 打开 AI 追问失败:", error);
    new ztoolkit.ProgressWindow("Self AI", {
      closeOnClick: true,
      closeTime: 3000,
    })
...
async function handleChatWithAI() {
  try {
    ...
    ztoolkit.log("[Self-AI] 打开聊天失败:", error);
    new ztoolkit.ProgressWindow("Self AI", {
      closeOnClick: true,
      closeTime: 3000,
    })
...
async function handleGenerateSummary() {
  ...
  const progressWin = new ztoolkit.ProgressWindow("Self AI", {
    closeOnClick: true,
    closeTime: 4000,
  });
  ...
  try {
    ...
    try {
      ...
      // 安全兜底，不影响后续流程
      ztoolkit.log("[Self-AI] 刷新任务队列视图失败:", e);
    }
    ...
    ztoolkit.log("[Self-AI] 入队失败:", error);
    progressWin
      .createLine({
...
async function onMainWindowUnload(win: Window): Promise<void> {
  // 移除文献库工具栏按钮
  try {
    const button = win.document.getElementById("self-ai-library-toolbar-btn");
    if (button) {
      button.remove();
    }
  } catch (e) {
...
async function handleImageSummary() {
  ...
  try {
    ...
    ztoolkit.log("[Self-AI] 添加一图总结任务失败:", error);
    new ztoolkit.ProgressWindow("Self AI", {
      closeOnClick: true,
      closeTime: 5000,
    })
...
async function handleMindmapGeneration() {
  ...
  try {
    ...
    ztoolkit.log("[Self-AI] 添加思维导图任务失败:", error);
    new ztoolkit.ProgressWindow("Self AI", {
      closeOnClick: true,
      closeTime: 5000,
    })
...
async function handleLiteratureReview() {
  try {
    ...
    ztoolkit.log("[Self-AI] 打开文献综述失败:", error);
    new ztoolkit.ProgressWindow("Self AI", {
      closeOnClick: true,
      closeTime: 5000,
    })
...
async function handleFillTable() {
  ...
  try {
    ...
    ztoolkit.log("[Self-AI] 填表失败:", error);
    new ztoolkit.ProgressWindow("Self AI", {
      closeOnClick: true,
      closeTime: 5000,
    })
...
async function handleMultiRoundSummary(
  ...
  try {
    ...
    ztoolkit.log("[Self-AI] 加入重分析队列失败:", error);
    new ztoolkit.ProgressWindow("Self AI", {
      closeOnClick: true,
      closeTime: 5000,
    })
